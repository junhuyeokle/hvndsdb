...
namespace GaussianSplatting.Runtime.Utils
{
    ...
    public static class SpzFileReader
    {
        ...
        static void ReadHeaderImpl(string filePath, Stream fs, out int vertexCount, out int shLevel, out int fractBits, out int flags)
        {
            ...
            var readBytes = fs.Read(NativeArrayHelper.AsSpan(header.Reinterpret<byte>(16)));
            if (readBytes != 16)
        ...
        public static void ReadFile(string filePath, out NativeArray<InputSplatData> splats)
        {
            ...
            bool readOk = true;
            readOk &= gz.Read(NativeArrayHelper.AsSpan(packedPos)) == packedPos.Length;
            readOk &= gz.Read(NativeArrayHelper.AsSpan(packedAlpha)) == packedAlpha.Length;
            readOk &= gz.Read(NativeArrayHelper.AsSpan(packedCol)) == packedCol.Length;
            readOk &= gz.Read(NativeArrayHelper.AsSpan(packedScale)) == packedScale.Length;
            readOk &= gz.Read(NativeArrayHelper.AsSpan(packedRot)) == packedRot.Length;
            readOk &= gz.Read(NativeArrayHelper.AsSpan(packedSh)) == packedSh.Length;
