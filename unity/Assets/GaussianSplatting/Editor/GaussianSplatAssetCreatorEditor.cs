...
{
    [BurstCompile]
    public class GaussianSplatAssetCreatorEditor : EditorWindow
    {
        ...
        {
            var window = GetWindowWithRect<GaussianSplatAssetCreatorEditor>(new Rect(50, 50, 360, 340), false, "Gaussian Splat Creator", true);
        ...
        static void CreateChunkData(NativeArray<InputSplatData> splatData, string filePath, ref Hash128 dataHash)
        {
            ...
            fs.Write(NativeArrayHelper.AsSpan(
                job.chunks.Reinterpret<byte>(UnsafeUtility.SizeOf<GaussianSplatAsset.ChunkInfo>())));
        ...
        void CreatePositionsData(NativeArray<InputSplatData> inputSplats, string filePath, ref Hash128 dataHash)
        {
            ...
            using var fs = new FileStream(filePath, FileMode.Create, FileAccess.Write);
            fs.Write(NativeArrayHelper.AsSpan(data));
        ...
        void CreateColorData(NativeArray<InputSplatData> inputSplats, string filePath, ref Hash128 dataHash)
        {
            ...
            if (GraphicsFormatUtility.IsCompressedFormat(gfxFormat))
            {
                ...
                using var fs = new FileStream(filePath, FileMode.Create, FileAccess.Write);
                fs.Write(NativeArrayHelper.AsSpan(cmpData));
            ...
            else
            {
                ...
                using var fs = new FileStream(filePath, FileMode.Create, FileAccess.Write);
                fs.Write(NativeArrayHelper.AsSpan(jobConvert.outputData));
        ...
        static void EmitSimpleDataFile<T>(NativeArray<T> data, string filePath, ref Hash128 dataHash) where T : unmanaged
        {
            dataHash.Append(data);
            using var fs = new FileStream(filePath, FileMode.Create, FileAccess.Write);
            fs.Write(NativeArrayHelper.AsSpan(data.Reinterpret<byte>(UnsafeUtility.SizeOf<T>())));
        }
