...
public:
    ...
    static handle cast(const type &src, return_value_policy policy, handle parent) {
        object first_obj = reinterpret_steal<object>(detail::make_caster<T1>::cast(src.first, policy, parent));
        object second_obj = reinterpret_steal<object>(detail::make_caster<T2>::cast(src.second, policy, parent));

        if (!first_obj || !second_obj)
            return handle();

        return make_tuple(std::move(first_obj), std::move(second_obj)).release();
    }
    ...
    static constexpr auto name = _("Tuple[") + make_caster<T1>::name + _(", ") + make_caster<T2>::name + _("]");

    template <typename U> using cast_op_type = type;
...
template <typename T1, typename T2>
class type_caster<std::pair<T1, T2>> : public pair_caster<T1, T2> {};
