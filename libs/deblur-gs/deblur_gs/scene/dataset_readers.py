...
def readColmapCameras(path, cam_extrinsics, cam_intrinsics, images_folder):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        test_image_path = os.path.join(
            path, "images_test", os.path.basename(extr.name))
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    cam_infos_unsorted = readColmapCameras(
        path, cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir))
