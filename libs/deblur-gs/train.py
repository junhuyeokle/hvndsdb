...
def training(
    ...
    while not is_open:
        retry = input(
            "visdom port ({}:{}) not open, retry? (y/n) ".format(
        ...
        if retry == "y":
            ...
            break
    vis = visdom.Visdom(server=dataset.visdom_server, port=dataset.visdom_port)
    ...
    blur_blend_embedding = torch.nn.Embedding(
        len(scene.getTrainCameras()), opt.blur_sample_num
    ).cuda()
    blur_blend_embedding.weight = torch.nn.Parameter(
        torch.ones(len(scene.getTrainCameras()), opt.blur_sample_num).cuda()
    )
    ...
    scheduler = torch.optim.lr_scheduler.ExponentialLR(
        optimizer, gamma=(1e-6 / 1e-3) ** (1.0 / opt.iterations)
    )
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        for idx in range(opt.blur_sample_num):
            ...
            render_pkg = render(
                viewpoint_cam, gaussians, pipe, bg, interp_alpha=alpha
            )
            (
                image_,
                depth_,
                viewspace_point_tensor_,
                visibility_filter_,
                radii_,
            ) = (
                render_pkg["render"],
            ...
            visibility_filter = (
                ...
                else torch.logical_or(visibility_filter, visibility_filter_)
            )
        ...
        loss = (1.0 - opt.lambda_dssim) * image_loss + opt.lambda_dssim * (
        ...
        if opt.depth_reg:
            loss += opt.depth_weight * min(
                ...
                (1 - pearson_corrcoef(1 / (predict_depth + 200.0), depth)),
            )

        loss.backward()

        viewspace_point_tensor = (
            viewspace_point_tensor_data.clone().detach().requires_grad_(True)
        )
...
def training_report(
    ...
    if tb_writer:
        tb_writer.add_scalar(
            "train_loss_patches/l1_loss", Ll1.item(), iteration
        )
        tb_writer.add_scalar(
    ...
    if iteration in testing_iterations:
        ...
        vis_cameras(
            vis, step=iteration, poses=[aligned_train_pose, train_pose_GT]
        )
        ...
        for config in validation_configs:
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx, viewpoint in enumerate(config["cameras"]):
                    ...
                    if tb_writer:
                        ...
                        if iteration == testing_iterations[0]:
                            tb_writer.add_images(
                                config["name"]
                                + "_view_{}/predict_depth".format(
                                    viewpoint.image_name
                                ),
                                vis_predict_depth[None],
                                global_step=iteration,
                            )
                            tb_writer.add_images(
                                config["name"]
                    ...
                    lpips_test += (
                        lpips(image, gt_image, net_type="vgg").mean().double()
                    )
                ...
                if tb_writer:
                    tb_writer.add_scalar(
                        config["name"] + "/loss_viewpoint - image_loss",
                        rgb_test,
                        iteration,
                    )
                    tb_writer.add_scalar(
                        config["name"] + "/loss_viewpoint - depth_loss",
                        depth_test,
                        iteration,
                    )
                    tb_writer.add_scalar(
                    ...
                    if config["name"] == "train":
                        ...
                        tb_writer.add_scalar(
                            config["name"] + "/loss_viewpoint - R_error",
                            cam_error.R.mean(),
                            iteration,
                        )
                        tb_writer.add_scalar(
                            config["name"] + "/loss_viewpoint - t_error",
                            cam_error.t.mean(),
                            iteration,
                        )
